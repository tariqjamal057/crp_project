...
class VendorPayment(TenantScopedModel):
    ...
    class Meta:
        ...
        unique_together = [('company', 'payment_number')]  # If system generated and unique per company
    ...
    def _recalculate_derived_fields(self, perform_save: bool = False, _triggering_save: bool = False):
        ...
        self.bill_allocations.all().aggregate(total=Coalesce(Sum('allocated_amount'), ZERO_DECIMAL))['total']
        self.unallocated_amount = (self.payment_amount or ZERO_DECIMAL) - self.allocated_amount

        # Status update logic is simple here; more complex workflows might exist
        # For example, if unallocated is > 0 after being COMPLETED, it might be an issue or overpayment.
        # This part of status update based on allocation is usually handled by the service that manages allocations.
        # For now, let's assume status is mainly driven by workflow (Draft -> Approved -> Paid/Completed -> Void)
        # and allocation amounts inform informational fields.

        if perform_save and not _triggering_save:
            update_fields = ['allocated_amount', 'unallocated_amount', 'updated_at']
            self.save(update_fields=list(set(update_fields)), _recalculating_payment=True)  # Pass flag
            logger.debug(f"VendorPayment PK {self.pk}: Saved with recalculated amounts.")

    def clean(self):  # clean for VendorPayment
        super().clean()
        errors = {}
        # (Company consistency checks similar to VendorBill.clean)
        effective_company: Optional[Company] = self.company
        if not effective_company and self.company_id:
            try: effective_company = Company.objects.get(pk=self.company_id)
            except Company.DoesNotExist: errors['company'] = _("Invalid Company ID.")
        ...
        if self.payment_amount <= ZERO_DECIMAL: errors['payment_amount'] = _("Payment amount must be positive.")
        if not self.currency:
            if effective_company and hasattr(effective_company, 'default_currency_code'):
                self.currency = effective_company.default_currency_code
            elif self.supplier and hasattr(self.supplier, 'default_currency') and self.supplier.default_currency:
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            if not self.currency:  # Default currency
                if self.supplier and hasattr(self.supplier, 'default_currency') and self.supplier.default_currency:
                    self.currency = self.supplier.default_currency
                elif self.company and hasattr(self.company, 'default_currency_code'):
                    self.currency = self.company.default_currency_code
            ...
            self.unallocated_amount = self.payment_amount or ZERO_DECIMAL  # Initially all unallocated
...
class VendorPaymentAllocation(TenantScopedModel):
    ...
    def __str__(self):
        p_id, b_id, amt = (self.vendor_payment_id or "N/A"), (self.vendor_bill_id or "N/A"), (
            self.allocated_amount if self.allocated_amount is not None else "N/A")
        return f"Alloc Pmt {p_id} to Bill {b_id} - Amt: {amt}"

    def clean(self):  # clean for VendorPaymentAllocation
        super().clean()
        errors = {}
        if self.allocated_amount is not None and self.allocated_amount <= ZERO_DECIMAL: errors['allocated_amount'] = _(
            "Allocated amount must be positive.")
        if not self.vendor_payment_id: errors['vendor_payment'] = _("Vendor Payment is required.")
        if not self.vendor_bill_id: errors['vendor_bill'] = _("Vendor Bill is required.")
        if errors: raise DjangoValidationError(errors)
        try:
            payment = VendorPayment.objects.select_related('company', 'supplier').get(pk=self.vendor_payment_id)
            bill = VendorBill.objects.select_related('company', 'supplier').get(pk=self.vendor_bill_id)
            # Set company on allocation from parent payment before validating other fields against it
            if not self.company_id and payment.company_id: self.company = payment.company

            if payment.company != bill.company: errors['vendor_bill'] = _(
                "Payment and Bill must be for the same company.")
            if payment.supplier != bill.supplier: errors['vendor_bill'] = _(
                "Payment and Bill must be for the same supplier.")
            if payment.currency != bill.currency: errors['vendor_bill'] = _(
                "Pmt currency (%(pc)s) must match Bill (%(bc)s).") % {'pc': payment.currency, 'bc': bill.currency}

            # More robust checks for over-allocation (best handled in service layer for atomicity with multiple allocations)
            # For model clean, this is a good basic check for a single allocation instance:
            # 1. Check against Bill's amount_due
            bill_due_before_this_alloc = bill.amount_due
            if self.pk:  # If updating, add back original amount of *this* allocation
                try:
                    bill_due_before_this_alloc += VendorPaymentAllocation.objects.values_list('allocated_amount',
                                                                                              flat=True).get(pk=self.pk)
                except VendorPaymentAllocation.DoesNotExist:
                    pass
            if self.allocated_amount > bill_due_before_this_alloc + Decimal('0.01'): errors['allocated_amount'] = _(
                "Applied (%(a)s) > Bill Due (%(d)s).") % {'a': self.allocated_amount, 'd': bill_due_before_this_alloc}

            # 2. Check against Payment's unallocated_amount
            payment_unallocated_before_this_alloc = payment.unallocated_amount
            if self.pk:  # If updating, add back original amount of *this* allocation
                try:
                    payment_unallocated_before_this_alloc += VendorPaymentAllocation.objects.values_list(
                        'allocated_amount', flat=True).get(pk=self.pk)
                except VendorPaymentAllocation.DoesNotExist:
                    pass
            if self.allocated_amount > payment_unallocated_before_this_alloc + Decimal('0.01'): errors[
                'allocated_amount'] = _("Applied (%(a)s) > Payment Unallocated (%(u)s).") % {'a': self.allocated_amount,
                                                                                             'u': payment_unallocated_before_this_alloc}

        except VendorPayment.DoesNotExist:
            errors['vendor_payment'] = _("Associated Vendor Payment not found.")
        except VendorBill.DoesNotExist:
            errors['vendor_bill'] = _("Associated Vendor Bill not found.")
        if errors: raise DjangoValidationError(errors)

    def save(self, *args, **kwargs):
        # Ensure company on allocation matches parent payment's company
        if self.vendor_payment_id and self.vendor_payment.company_id:
            self.company_id = self.vendor_payment.company_id

        if not kwargs.pop('skip_clean', False): self.full_clean()
        super().save(*args, **kwargs)
        # Trigger recalculations on parent Payment and Bill.
        # This needs to be done carefully to avoid excessive saves or race conditions.
        # Signals or service layer control is generally better.
        # For direct model save impact:
        if self.vendor_payment_id:
            try:
                VendorPayment.objects.get(pk=self.vendor_payment_id)._recalculate_derived_fields(perform_save=True)
            except VendorPayment.DoesNotExist:
                pass
        if self.vendor_bill_id:
            try:
                VendorBill.objects.get(pk=self.vendor_bill_id)._recalculate_derived_fields(perform_save=True)
            except VendorBill.DoesNotExist:
                pass
    ...
    def delete(self, *args, **kwargs):
        ...
        if payment_to_update_pk:
            try:
                VendorPayment.objects.get(pk=payment_to_update_pk)._recalculate_derived_fields(perform_save=True)
            except VendorPayment.DoesNotExist:
                pass
